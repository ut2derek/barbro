...
-- --- właściwe liczenie wolnych terminów ---
--
-- Cała logika siedzi tutaj, bez sprawdzania, kto pyta. Kontrolę dostępu robi
-- `get_available_slots` poniżej — dzięki temu warunek jest w jednym miejscu
-- i nie da się go pominąć, wołając funkcję inną drogą. Tej funkcji celowo
-- nie udostępniamy zalogowanym użytkownikom.

create or replace function public.available_slots_unchecked(
  p_salon_id uuid,
  p_service_ids uuid[],
  p_from date,
  p_to date,
...
begin
  ...
  durations as (
    select
      ...
      ) + greatest(coalesce(p_extra_minutes, 0), 0) as visit_minutes,
...
-- Bez usunięcia poprzedniej wersji Postgres ma dwie funkcje o tej samej
-- nazwie i wywołanie bez ostatniego argumentu staje się niejednoznaczne.
