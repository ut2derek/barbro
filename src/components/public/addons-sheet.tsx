...
export function AddonsSheet({
  ...
  const total = addons.reduce((sum, addon) => sum + (chosen[addon.id] ?? 0) * addon.priceGrosz, 0);

  return (
    <Sheet visible={visible} onClose={onClose} padded={false} style={{ paddingTop: theme.spacing.lg }}>
