...
export function useUpdateSalonSettings() {
  ...
  return useMutation({
    mutationFn: async (args: { salonId: string; changes: Partial<SalonSettings> }) => {
      ...
      const payload: TablesUpdate<'salons'> = {};
    ...
    // Siatka, wyprzedzenie, horyzont, strefa i wygląd zmieniają też to, co
    // widzi klient na stronie rezerwacji — pełna lista jest w `query-keys.ts`.
